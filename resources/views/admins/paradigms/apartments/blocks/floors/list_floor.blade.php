        ...
        <div class="container mb-2">
            <div class="mt-2">
                <fieldset class="scheduler-border">
                    ...
                    <form action="{{route('paradigms-floors.index', ['paradigm_id' => 1])}}" method="GET">
                        @method('GET')
                        @csrf
            ...
            <div class="mt-3">
                <div class="d-flex justify-content-between">
                    ...
                    <div>
                        <button type="button" class="btn btn-info btn-sm" id="modal_create_floor_btn">
            ...
            <div id="list_apartment_table">
                @include('admins.paradigms.apartments.blocks.floors.ajax_list_floor')
            </div>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="{{route('paradigms-floors.store', ['paradigm_id' => 1])}}" method="POST">
                                @method('POST')
                                @csrf
                                <div class="row">
                                    <div class="col-3">
                                    ...
                                    <div class="col-9">
                                        <select name="apartment_id" onchange="getBlockByApartmentIdCrud(this.value, '{{$paradigm_id}}', '{{$url}}', 'create')" class="custom-select">
                                            @if ($apartments && count($apartments) > 0)
                                                @foreach ($apartments as $apartment)
                                                    <option class="option_apartment" value="{{$apartment->id}}" {{isset($params['apartment_id']) ? $params['apartment_id'] == $apartment->id ? 'selected' : '' : ''}}>{{$apartment->name}}</option>
                                                @endforeach
                                    ...
                                    <div class="col-9 mt-3">
                                        <select name="block_id" id="block_id_create" class="custom-select">
                                            @if ($blocks->where('apartment_id', $apartment->first()->id) && $blocks->where('apartment_id', $apartment->first()->id)->count() > 0)
                                                @foreach ($blocks->where('apartment_id', $apartment->first()->id) as $block)
                                                    <option class="option_block" value="{{$block->id}}" {{isset($params['block_id']) ? $params['block_id'] == $block->id ? 'selected' : '' : ''}}>{{$block->name}}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    ...
                                    <div class="col-3 mt-3">
                                        <label for="floor_name_create" class="col-form-label float-right">Tên Tầng:</label>
                                    </div>
                                    <div class="col-9 mt-3">
                                        <input type="text" name="name" id="floor_name_create" class="form-control form-control-sm" required>
                                        <input type="text" name="paradigm_id" value="{{$paradigm_id ?? 0}}" hidden>
                                    </div>
                                </div>
            ...
            <div class="modal fade" id="modal_update_floor" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel_update" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel_update">Sửa Tầng</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body" id="update_content_floor">
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        $(document).off('click', '#modal_create_floor_btn').on('click', '#modal_create_floor_btn', function(e) {
            e.preventDefault();
            $('#modal_create_floor').modal('show');
        });
    });

    function getFloor(paradigm_id, id, url) {

        $('#modal_update_floor').modal('show');

        let content_modal_update_floor =  $('#update_content_floor');
        ...
        $.ajax({
            ...
            url: '{{route('paradigms-floors.edit','id')}}',

            success: function (data) {

                content_modal_update_floor.html(data);
