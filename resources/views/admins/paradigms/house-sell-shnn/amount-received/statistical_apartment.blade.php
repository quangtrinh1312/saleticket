    ...
    <div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline">
        <div class="d-flex">
            <div class="breadcrumb">
                ...
                <a href="alpaca_advanced.html" class="breadcrumb-item active">Thống kê thu tiền (Chung cư)</a>
        ...
        <div class="container">
            <div class="mt-2">
                <fieldset class="scheduler-border">
                    ...
                    <form action="{{route('get.paradigms.house.sell.shnn.statistical', ['paradigm_id' => 4])}}" method="GET">
                        @method('GET')
                        @csrf
                        <div class="row">
                            <div class="col-6">
                                <div class="form-group">
                                    <div class="row align-items-center">
                                        <div class="col-4">
                                            <label for="" class="float-right col-form-label">Khu chung cư</label>
                                        </div>
                                        <div class="col-8">
                            ...
                            <div class="col-6">
                                <div class="form-group">
                                    <div class="row align-items-center">
                                        <div class="col-4">
                                            <label for="" class="float-right col-form-label">Căn hộ</label>
                                        </div>
                                        <div class="col-8">
                                            <input list="" name="name" value="{{$params['name'] ?? ''}}" class="form-control form-control-sm">
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    <div class="row align-items-center">
                                        ...
                                        <div class="col-8">
                                            ...
                                            <!-- <select name="account_id" class="custom-select">
                                                <option value="">Tất cả</option>
                                                @if ($accounts && count($accounts) > 0)
                                                    @foreach ($accounts as $account)
                                                        <option class="option_account" value="{{$account->id}}" {{isset($account_id) ? $account_id == $account->id ? 'selected' : '' : ''}}>{{$account->fullname}}</option>
                                ...
                                <div class="form-group">
                                    <div class="row align-items-center">
                                        ...
                                        <div class="col-8">
                                            ...
                                            <input type="text" name="household_type" value="1" hidden>
            ...
            <a href="{{ route('get.paradigms.export.excel', ['paradigm_id' => 4, 'params' => $params, 'household_type' => 1]) }}" class="btn btn-success btn-sm">
            ...
            <div class="mt-1 row">
                <div class="col-lg-12">
                    ...
                    <form action="{{route('confirm.send.accountant.house.sell.shnn')}}" method="POST">
                    @method('POST')
                    @csrf
                    <input type="text" name="paradigm_id" value="{{$paradigm_id ?? 0}}" hidden>
            ...
            <div id="">
                    ...
                    <table class="table table-hover table-dark table-striped mb-1 table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="text-center" style="width: 170px !important;">Khu chung cư</th>
                                <th scope="col" class="text-center" style="width: 100px !important;">Căn hộ</th>
                        ...
                        <tbody id="list_accountant_table_data">
                            ...
                            <tr id="r_id_">
                                ...
                                <td scope="row">{{$sn ++}}</td>
                                <td scope="row">{{getNameApartmentByID($amountReceived_1_->apartment_id) ?? ''}}</td>
                                <td scope="row">{{getNameRoomByID($amountReceived_1_->room_id) ?? ''}}</td>
                                ...
                                <td scope="row">{{getNameTypePriceByID($amountReceived_1_->type_price_id) ?? ''}}</td>
                                ...
                                <td scope="row">
                                    ...
                                    @if ($amountReceived_1_->send_accountant == 1)
                                        <a href="{{route('paradigms-apartments.collect.cancel.send.accountant',['id' => $amountReceived_1_->id, 'paradigm_id' => 4])}}" class="badge badge-danger "
                                                onclick="if(!confirm('Bạn có chắc chắn muốn hủy ?')){return false}" >
                                            <i class="icon-cross"></i></a>
                                    @endif
                                </td>
                                <td scope="row" class="text-center">
                                    <!-- <form action="{{route('paradigms-apartments.collect.delete')}}"
                                    method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <input type="text" name="paradigm_id" value="{{$paradigm_id ?? 0}}" hidden>
                                    ...
                                    <a href="{{route('paradigms-apartments.house.sell.shnn.collect.delete',['id' => $amountReceived_1_->id])}}" class="badge badge-danger "
                                                onclick="if(!confirm('Bạn có chắc chắn muốn hủy ?')){return false}" >
                                            <i class="icon-cross"></i></a>
                                    @endif
                                </td>
                ...
                <!--                 <li class="page-item {{ $amountReceived_1_s->currentPage() == 1 ? 'disabled' : '' }}">
                                ...
                                @if (isset($amountReceived_1_s) && count($amountReceived_1_s) > 0)
                                    {!! $amountReceived_1_s->appends(Request::all())->render('helpers.paginate') !!}
                                @endif
                        ...
                        <div class="modal-body">
                            <div class="mt-2 table-wrapper">
                                <table class="table table-hover table-dark table-striped table-bordered mb-1">
                                    <thead>
                                        <tr>
                                            <th scope="col" class="text-center" style="max-width: 50px !important;">STT</th>
                                            <th scope="col" class="text-center" style="width: 250px !important;">Quản lý viên</th>
                                            <th scope="col" class="text-center" style="width: 170px !important;">Tiền mặt</th>
                                            <th scope="col" class="text-center" style="width: 170px !important;">Chuyển khoản</th>
                                            <th scope="col" class="text-center" style="width: 170px !important;">Tổng cộng</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td scope="row">{{getNameAccountByID($result->account_paid) ?? ''}}</td>
                                            <td scope="row" class="text-right">{{number_format($result->cash,2,',','.')}}</td>
                                            <td scope="row" class="text-right">{{number_format($result->transfer,2,',','.')}}</td>
                                            <td scope="row" class="text-right">{{number_format($result->sum,2,',','.')}}</td>
                                        </tr>
                                        @endforeach
                                        @endif
                                        <!-- <tr>
