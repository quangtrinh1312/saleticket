<div>
    <div wire:ignore.self class="modal fade" id="modal_create_floor" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="modal-body">
                    <form wire:submit.prevent="create">
                        <div class="row">
                            ...
                            <div class="col-9">
                                <div wire:ignore>
                                    <select class="custom-select select-create-floor" id="select-apartment-create-floor">
                            ...
                            <div class="col-9 mt-3">
                                <div wire:ignore>
                                    <select class="custom-select select-create-floor" id="select-block-create-floor">
...
<script>
    $(document).ready(function() {
        $('.select-create-floor').select2({
            placeholder: '‎',
            allowClear: false,
        });

        $('#select-apartment-create-floor').on('change', function(e) {
            var value = $('#select-apartment-create-floor').select2("val");
            @this.set('apartmentId', value);
        })

        $('#select-block-create-floor').on('change', function(e) {
            var value = $('#select-block-create-floor').select2("val");
            @this.set('blockId', value);
        })
    });

    window.addEventListener('openCreateFloor', () => {
        $('#modal_create_floor').modal('show');
    })

    window.addEventListener('closeCreateFloor', () => {
        $('#modal_create_floor').modal('hide');
    })

    window.addEventListener('updateCreateFloorSelectBox', (e) => {
        var list = @this.get(e.detail + 'Arr')
        var selectedItemId = @this.get(e.detail + 'Id')
        var selectBox = $('#select-' + e.detail + '-create-floor')
