<div>
    <div wire:ignore.self class="modal fade" id="modal_create_block" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="modal-body">
                    <form wire:submit.prevent="create">
                        <div class="row">
                            ...
                            <div class="col-9">
                                <div wire:ignore>
                                    <select class="custom-select select-create-block" id="select-apartment-create-block">
...
<script>
    $(document).ready(function() {
        $('.select-create-block').select2({
            placeholder: '‎',
            allowClear: false,
        });

        $('#select-apartment-create-block').on('change', function(e) {
            var value = $('#select-apartment-create-block').select2("val");
            @this.set('apartmentId', value);
        })
    });

    window.addEventListener('openCreateBlock', () => {
        $('#modal_create_block').modal('show');
    })

    window.addEventListener('closeCreateBlock', () => {
        $('#modal_create_block').modal('hide');
    })

    window.addEventListener('updateCreateBlockSelectBox', (e) => {
        var list = @this.get(e.detail + 'Arr')
        var selectedItemId = @this.get(e.detail + 'Id')
        var selectBox = $('#select-' + e.detail + '-create-block')
