        ...
        <div class="container">
            <div class="mt-2">
                <fieldset class="scheduler-border">
                    ...
                    <form action="{{route('get.paradigms.dor.statistical', ['paradigm_id' => 7])}}" method="GET">
                        @method('GET')
                        @csrf
                        <div class="row">
                            <div class="col-6">
                                <div class="form-group">
                                    <div class="row align-items-center">
                                        <div class="col-4">
                                            <label for="" class="float-right col-form-label">Khu KTX</label>
                                        </div>
                                        <div class="col-8">
                                            <select name="dormitory_id" class="custom-select">
                                                ...
                                                @if ($apartments && count($apartments) > 0)
                                                    @foreach ($apartments as $apartment)
                                                        <option class="option_apartment" value="{{$apartment->id}}" {{isset($params['dormitory_id']) ? $params['dormitory_id'] == $apartment->id ? 'selected' : '' : ''}}>{{$apartment->name}}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
            ...
            <a href="{{ route('get.paradigms.export.excel', ['paradigm_id' => 7, 'params' => $params]) }}" class="btn btn-success btn-sm">
            ...
            <div class="mt-1 row">
                <div class="col-lg-12">
                    ...
                    <!-- <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#modal_statistic">
                            <i class="fa-solid fa-chart-simple mr-1"></i>
                            Xem thống kê
                    </button> -->
                    <form action="{{route('confirm.send.accountant.ktx')}}" method="POST">
                    @method('POST')
                    @csrf
                    <input type="text" name="paradigm_id" value="{{$paradigm_id ?? 0}}" hidden>
                    ...
                    <table class="table table-hover table-dark table-striped mb-1 table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="text-center" style="width: 170px !important;">Địa chỉ <br>(khu KTX)</th>
                        ...
                        <tbody id="list_accountant_table_data">
                            ...
                            <tr id="r_id_">
                                ...
                                <td scope="row">{{$amountReceived_1_->dor->name}}</td>
                                ...
                                <td scope="row" class="text-center">
                                    ...
                                    @if ($amountReceived_1_->send_accountant == 1)
                                        <a href="{{route('paradigms-apartments.collect.cancel.send.accountant',['id' => $amountReceived_1_->id, 'paradigm_id' => 7])}}" class="badge badge-danger "
                                ...
                                <td scope="row" class="text-center">
                                    ...
                                    <a href="{{route('paradigms-dors.collect.delete',['id' => $amountReceived_1_->id])}}" class="badge badge-danger "
                                                onclick="if(!confirm('Bạn có chắc chắn muốn hủy ?')){return false}" >
                                            <i class="icon-cross"></i></a>
                                    @endif
