        ...
        <div class="container">
            <div class="mt-2">
                <fieldset class="scheduler-border">
                    ...
                    <form action="{{route('get.paradigms.dor.history',  ['paradigm_id' => 7])}}" method="GET">
                        @method('GET')
                        @csrf
                        <div class="row">
                            <div class="col-6">
                                            ...
                                            <select name="account_id" class="custom-select">
                                                ...
                                                @if ($accounts && count($accounts) > 0)
                                                    @foreach ($accounts as $account)
                                                        <option class="option_account" value="{{$account->id}}" {{isset($params['account_id']) ? in_array($account->id, $params['account_id']) ? 'selected' : '' : ''}}>{{$account->fullname}}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
...
<!--             <div class="mt-1 row">
                <div class="col-lg-12">
                	<a href="{{$url}}" class="btn btn-success"><i class="icon-file-excel"></i> Xuất Excel</a>
                </div>
            </div> -->
...
<script type="">
    ...
    function getApartment(paradigm_id, id, url) {
        ...
        $.ajax({
            ...
            url: '{{route('paradigms-dors.collect.edit','id')}}',
    ...
    function getDetail(paradigm_id, id, url) {
        ...
        $.ajax({
            ...
            url: '{{route('paradigms-dors.collect.detail','id')}}',
